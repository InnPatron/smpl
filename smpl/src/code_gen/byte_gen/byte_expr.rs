    ...
    let single = match *value.data() {
        Value::Literal(ref lit) => match *lit {
            Literal::String(ref string) => {
                Store(Location::Tmp(store), Arg::String(string.to_string()))
            }

            Literal::Int(int) => Store(Location::Tmp(store), Arg::Int(int)),

            Literal::Float(float) => {
                Store(Location::Tmp(store), Arg::Float(float))
            }

            Literal::Bool(boolean) => {
                Store(Location::Tmp(store), Arg::Bool(boolean))
            }
        },
        ...
        Value::Binding(ref var) => {
            ...
            Store(
                Location::Tmp(store),
                Arg::Location(Location::Namespace(value)),
            )
        }
        ...
        Value::FieldAccess(ref access) => {
            ...
            let root_var = internal_path.root_name().data().as_str().to_owned();
            let root_indexing_expr = internal_path.root_indexing_expr();

            if let Some(root_indexing_expr) = root_indexing_expr {
                instruction_buffer
                    .extend(translate_expr(root_indexing_expr, typing_context));
            }
            ...
            let path: Vec<_> = internal_path
                ...
                .map(|path_segment| match path_segment {
                    ...
                    PathSegment::Indexing(ref field, ref index_expr) => {
                        instruction_buffer
                            .extend(translate_expr(index_expr, typing_context));
            ...
            let field_access_location =
                if path.len() > 0 || root_indexing_expr.is_some() {
                    Location::Compound {
                        root: root_var,
                        root_index: root_indexing_expr
                            .map(|expr| tmp_id(expr.last())),
                        path: path,
                    }
                } else {
            ...
            let access_instr = Store(
                Location::Tmp(store),
                Arg::Location(field_access_location),
            );
        ...
        Value::BinExpr(ref op, ref lhs, ref rhs) => {
            ...
            match op {
                ...
                BinOp::GreaterEq => specific_math_op!(
                    ty,
                    Location::Tmp(store),
                    lhs,
                    rhs,
                ...
                BinOp::LesserEq => specific_math_op!(
                    ty,
                    Location::Tmp(store),
                    lhs,
                    rhs,
                ...
                BinOp::Greater => specific_math_op!(
                    ty,
                    Location::Tmp(store),
                    lhs,
                    rhs,
        ...
        Value::StructInit(ref struct_init) => {
            ...
            for (field, typed_tmp_id) in field_init.into_iter() {
                ...
                map.insert(
                    field.to_string(),
                    Arg::Location(Location::Tmp(tmp_id(tmp))),
                );
            }
        ...
        Value::ArrayInit(ref array_init) => match array_init {
